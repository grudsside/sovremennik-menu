(() => {
    ...
    return `
      <details class="revision-manual revision-correction" id="${EDITOR_ID}">
        ...
        <div class="revision-correction-note">
          ...
          Выберите дату, исправьте ошибочные значения и обязательно укажите причину. Дата ревизии и сотрудник, который её отправил, сохраняются без изменений.
        </div>
        <form class="revision-form" id="${FORM_ID}">
          <div class="form-grid revision-correction-grid">
            <label class="employee-field">Ревизия за дату
              <select name="revisionDate" required>
          ...
          <label class="employee-field revision-correction-reason">Причина исправления
            <textarea name="reason" rows="3" maxlength="500" required placeholder="Например: сотрудник ошибся при снятии веса бункера"></textarea>
          </label>
  ...
  async function submitCorrection(event) {
    ...
    if (!isRevisionAdmin()) {
      setStatus(form, 'Исправлять ревизии может только администратор.', true);
      return;
    }
    if (!revisionDate) {
      setStatus(form, 'Выберите ревизию.', true);
      return;
    }
    if (!reason) {
    ...
    if (Number.isNaN(writeOffs) || Number.isNaN(iikoSales) || (writeOffs !== null && writeOffs < 0) || (iikoSales !== null && iikoSales < 0)) {
      setStatus(form, 'Списания и продажи должны быть положительными числами или оставаться пустыми.', true);
      return;
    }

    setStatus(form, 'Сохраняю исправление…');
    ...
    try {
      ...
      setStatus(form, 'Исправление сохранено и записано в историю.');
      form.elements.reason.value = '';
      fillCorrectionForm(form, revisionDate);
      await loadRevisionEditHistory(form.closest(`#${EDITOR_ID}`), true);
      alert('Ревизия исправлена');
    } catch (error) {
      console.error('Coffee revision correction failed', error);
      const message = error?.message || 'Проверьте интернет и права доступа.';
      setStatus(form, `Не удалось сохранить исправление: ${message}`, true);
    } finally {
  ...
  function renderHistory(entries) {
    ...
    return `<div class="revision-history-list">${entries.map(entry => {
      const dateLabel = typeof displayDateFromKey === 'function'
        ? displayDateFromKey(entry.revision_date)
        : entry.revision_date;
      const createdLabel = typeof formatDateTime === 'function'
        ? formatDateTime(entry.created_at)
        : entry.created_at;
      return `
        <article class="revision-history-item">
          <div class="revision-history-head">
            <strong>${safeText(dateLabel)}</strong>
            <span>${safeText(createdLabel)}</span>
          </div>
  ...
  function bindEditor(editor) {
    ...
    editor.addEventListener('toggle', () => {
      if (editor.open) loadRevisionEditHistory(editor);
    });
  }
  ...
  if (panels) {
    new MutationObserver(scheduleEnsure).observe(panels, { childList: true, subtree: true });
  }
