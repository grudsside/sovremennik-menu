...
export function setupWebPush() {
  ...
  const subject = Deno.env.get('VAPID_SUBJECT') || 'mailto:admin@sovremennik.local';
  if (!publicKey || !privateKey) throw new Error('VAPID keys are not configured');
  webpush.setVapidDetails(subject, publicKey, privateKey);
}

export async function listAdminManagerIds(supabase: ReturnType<typeof adminClient>) {
  const { data, error } = await supabase.from('profiles').select('id').eq('is_active', true).in('role', ['admin', 'manager']);
  if (error) throw error;
  return (data || []).map((r: any) => r.id);
}

export async function listAllActiveIds(supabase: ReturnType<typeof adminClient>) {
  const { data, error } = await supabase.from('profiles').select('id').eq('is_active', true);
  if (error) throw error;
  return (data || []).map((r: any) => r.id);
}

export async function loadPrefs(supabase: ReturnType<typeof adminClient>, userId: string) {
...
export async function sendPushToUsers(params: {
  ...
  for (const userId of uniqueUserIds) {
    ...
    const { data: existing } = await supabase.from('notification_events').select('id,status').eq('user_id', userId).eq('event_key', eventKey).maybeSingle();
    if (existing) { results.push({ userId, skipped: true, reason: 'duplicate' }); continue; }
    const { data: eventRow, error: eventError } = await supabase.from('notification_events').insert({
      user_id: userId,
    ...
    if (eventError) { results.push({ userId, error: eventError.message }); continue; }

    const { data: subs, error: subsError } = await supabase.from('push_subscriptions').select('*').eq('user_id', userId).eq('is_active', true);
    if (subsError) { results.push({ userId, error: subsError.message }); continue; }
    if (!subs?.length) {
      await supabase.from('notification_events').update({ status: 'no_subscription' }).eq('id', eventRow.id);
    ...
    for (const sub of subs) {
      try {
        const subscription = sub.subscription_json || { endpoint: sub.endpoint, keys: { p256dh: sub.p256dh_key, auth: sub.auth_key } };
    ...
    await supabase.from('notification_events').update({ status: sent ? 'sent' : 'error', error: sent ? null : lastError, sent_at: sent ? new Date().toISOString() : null }).eq('id', eventRow.id);
