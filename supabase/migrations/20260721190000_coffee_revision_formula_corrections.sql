...
-- If the newer stock-aware report is already installed, preserve it instead of
-- attempting to replace it with an older, shorter column list.
...
begin
  if exists (
    ...
    raise notice 'Stock-aware coffee revision report already installed; formula-only view replacement skipped.';
    return;
  end if;
...
comment on view public.coffee_revision_report is
  'Coffee revision calculations: write-offs plus negative unaccounted discrepancy are total losses; newer stock-aware versions are preserved.';
