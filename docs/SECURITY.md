...
## Notify-event source authorization

`notify-event` работает через service-role client, поэтому RLS не заменяет серверную проверку источника события.

Обязательные правила:

- `task_assigned` разрешён только создателю задачи;
- `task_completed` разрешён исполнителю задачи либо активному `admin`/`manager`, и только когда задача уже имеет статус `done`;
- `checklist_submitted`, `revision_submitted` и `error_report_submitted` разрешены только сотруднику, указанному владельцем исходной записи;
- `schedule_event_added` разрешён только активному `admin`/`manager`, который создал исходное событие;
- исходная запись должна существовать в соответствующей таблице;
- получатели, заголовок, текст и dedupe key строятся из данных базы, а не из переданных клиентом имени, названия или даты;
- отсутствие идентификатора источника отклоняется, а не заменяется случайным dedupe key.

Проверка существования, состояния и владельца источника выполняется до создания `notification_events` и отправки Web Push.
...
- Deno format, active-profile и notify-event authorization tests, а также type-check для `push-send` и `notify-event`.

Не ослаблять проверки и не добавлять исключения ради прохождения CI без отдельного обоснованного security review. Ruleset `Protect main` должен блокировать merge до успешного `Repository checks / checks`.
