...
3. Built-in admin и local token fallback.
...
Built-in admin, migrations, deployments, overrides, RLS, menu sources, cron, public repo/secrets.

### 4. Вопросы

Только существенные. Начни с production URL, applied SQL, deployed Functions, cron, manager, built-in admin и следующего приоритета.
