...
const required = [
  ...
  [resolver.includes("if (/service[_ -]?role/.test(text)) return 110;"), 'preview auth administration must prefer the legacy service_role JWT'],
  ...
  [smoke.includes('non-admin role change rejected') || smoke.includes('non-admin role change Edge request rejected'), 'live test must verify role authorization'],
  [smoke.includes('non-admin direct role RPC rejected'), 'live test must reject direct role RPC calls'],
  [smoke.includes('role change audit persisted'), 'live test must verify role change audit data'],
  [smoke.includes('role-change notification'), 'live test must verify role change notification'],
  [smoke.includes('last active admin protected by database trigger'), 'live test must verify direct-write last-admin protection'],
  ...
  [coffeeSmoke.includes('barista duplicate operational overwrite rejected') && coffeeSmoke.includes('admin direct operational overwrite rejected'), 'live test must reject duplicate operational overwrites'],
  ...
  [migrations.includes('20260724110000_checklist_template_editor_preview.sql'), 'preview migration bundle must include checklist template editing'],
