...
const controlMarkup = active => `
<section id="top-control" class="top-panel active">
  ...
  <div id="control-revisions" class="control-folder ${active === 'revisions' ? 'active' : ''}">
...
const markups = { revisions:controlMarkup('revisions'), checklists:controlMarkup('checklists') };
const pageHtml = `<!doctype html><html><head><meta charset="utf-8"><style>body{margin:0}.control-folder:not(.active){display:none}summary{min-height:48px;display:flex;align-items:center;cursor:pointer}</style></head><body><div id="app">${markups.revisions}</div></body></html>`;
...
await page.evaluate(rows => {
  window.__controlMarkups = rows;
  window.__activeControlFolder = 'revisions';
  window.renderApp = () => { document.querySelector('#app').innerHTML = window.__controlMarkups[window.__activeControlFolder]; };
  window.refreshControl = () => {
    const current = document.querySelector('#top-control');
    current.outerHTML = window.__controlMarkups[window.__activeControlFolder];
  };
}, markups);

await page.addScriptTag({ path:path.join(root, 'assets/js/checklist-ui-state-fix.js') });
await page.addScriptTag({ path:path.join(root, 'assets/js/control-revision-scroll-fix.js') });
...
// Switch to checklist history and keep the same visible content during repeated full Control redraws.
await page.evaluate(() => {
  window.__activeControlFolder = 'checklists';
  window.refreshControl();
});
...
// Return to revisions and confirm its state survived redraws while the folder was hidden.
await page.evaluate(() => {
  window.__activeControlFolder = 'revisions';
  window.refreshControl();
});
await page.waitForFunction(() => document.querySelector('[data-revision-id]')?.open === true);
assert.equal(await page.locator('[data-revision-id]').evaluate(node => node.open), true, 'Expanded revision must keep its state across Control folder redraws');
