...
const required = [
  ...
  [push.includes('checklist-ui-state-fix.js?v=20260725-3'), 'submitted checklist toggle state fix must be loaded'],
  ...
  [serviceWorker.includes("sovremennik-offline-20260725-v12"), 'PWA cache must be refreshed'],
  ...
  [stateFix.includes('function managedControlSummary') && stateFix.includes(".checklist-submission-details[data-checklist-submission],.control-details"), 'submitted checklist summaries must use the stable delegated toggle path'],
  ...
  [stateFix.includes('if(!event.target.isConnected || !document.documentElement.contains(event.target)) return;'), 'detached toggle events must not overwrite submitted checklist state'],
