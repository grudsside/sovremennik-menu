...
const required = [
  ...
  [push.includes('control-section-draft-key-bridge.js?v=20260725-2'), 'offline checklist key bridge v2 must be loaded'],
  ...
  [serviceWorker.includes("sovremennik-offline-20260725-v18"), 'PWA cache must be refreshed'],
  ...
  [draftKeyBridge.includes('drafts.set = function') && draftKeyBridge.includes('input?.dataset?.task') && draftKeyBridge.includes('input?.dataset?.photoItemKey'), 'offline draft key bridge must preserve both raw and photo-enhanced checkbox keys'],
  [draftKeyBridge.includes('openRestoredDraft') && draftKeyBridge.includes("data-offline-restored=\"1\""), 'restored offline drafts must reopen their checklist card'],
